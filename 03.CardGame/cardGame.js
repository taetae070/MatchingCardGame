document.addEventListener('DOMContentLoaded', () => {
    ...
    //걸린시간 화면에 1초마다 업데이트
    /*
    겪은 문제점: 카드 다 뒤집었을 때 elapsedTime이 자꾸 0으로 기록됨
    원인: elapsedTime가 startGameTimer안에서만 업데이트되고 있기때문이었다.
    해결방법: 게임이 끝났을 때의 최종 시간을 finalTime 변수에 따로 저장했다.
    */
    ...
    원인: 카드를 뒤집어야하는데 카드요소가 아닌 '이미지 경로'를 저장한 filppedCards배열만 사용했다. 즉, 클릭 시 카드 요소를 비교할 수 있는 비교군 자체가 없어서 초기화가 안됐던 것이었다.
    해결방법: 클릭한 카드의 DOM 요소 저장해주는 flippedCardElements배열을 추가함.
    ...
    */
    function handleCardClick(Front, Back, cardInner){
        rounds++;
    ...
    Fronts.forEach((Front, index) => {
        ...
        Front.addEventListener('click', () => {
            handleCardClick(Front, Back, cardInner);
        });
    });
});